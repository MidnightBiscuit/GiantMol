...
integer                 :: brng, method ,seed0, i, j_store_aux
...
!dir$ if(save_trj_cloud .eq. 1)
    allocate(save_trj_Cloud(n_ions1,3,floor(j_max / real(n_save_temp)) + 2))
!dir$ endif
    ! zero step:
...
!~     call initialize_ions_LC()
    ...
    j_end = i_laser_fly*n_dt - modulo(j_start + i_laser_fly*n_dt -1,n_dt) + int(0.25*n_dt)
    j_store_aux = j_end
    allocate(save_temperature(floor(j_end / real(n_save_temp)) + 2, 0:7))
!dir$ if(save_trj_cloud .eq. 1)
    allocate(save_trj_Cloud(n_ions1,3,floor(j_end / real(n_save_temp)) + 2))
!dir$ endif
...
implicit none
    if (n_save_temp == i_save_temp) then
        ...
        !dir$ if ( save_trj_cloud .eq. 1 )
        save_trj_Cloud(:,1,j_save_temp) =  sngl(r_x(:n_ions1))
        save_trj_Cloud(:,2,j_save_temp) =  sngl(r_y(:n_ions1))
        save_trj_Cloud(:,3,j_save_temp) =  sngl(r_z(:n_ions1))
        !dec$ endif
    else
...
!--------- Saving the Cloud Evolution -----------------------
    open(unit = 10, status='replace',file=trim(adjustl(str_file_xva))//'_TrjN.bin',form='unformatted')  ! create a new file, or overwrite an existing one
    write(10) j_save_temp
    write(10) n_ions(1)
    write(10) save_trj_Cloud
    close(10)
...
!dir$ elseif(Simu_type .eq. 4)
	write(str_stat,"(I2.2)") i
    str_file_aux = 'SimuType0_'//trim(adjustl(str_stat)) &
                // trim(adjustl(str_N_aux))    &
                // trim(adjustl(str_Vrf_aux))  &
                // trim(adjustl(str_Udc_aux))  &
                // trim(adjustl(str_D_aux))    &
